...
it("Will construct a distance calculator", function () {
    var calculator = new DistanceCalculator_1.DistanceCalculator([]);
    expect(calculator).toBeInstanceOf(DistanceCalculator_1.DistanceCalculator);
});
it("Will calculate the distance between nodes", function () {
    var nodes = [new Node_1.Node(0, 0), new Node_1.Node(3, 4)];
    var calculator = new DistanceCalculator_1.DistanceCalculator(nodes);
...
it("Will determine its greatest and least x and y values", function () {
    var nodes = [new Node_1.Node(0, 5), new Node_1.Node(3, 10), new Node_1.Node(10, 0)];
    var calculator = new DistanceCalculator_1.DistanceCalculator(nodes);
...
it("Will assign nodes to their grid cells", function () {
    var nodes = [new Node_1.Node(1, 1), new Node_1.Node(2, 2), new Node_1.Node(3, 3)];
    var grid = {
        xMin: 1,
        xMax: 3,
        yMin: 1,
...
it("Will find the closest nodes in the grid", function () {
    ...
    expect(calculator.collectNodesCloseToNodeInGrid(new Node_1.Node(1, 0), grid)).toStrictEqual([nodes[0], nodes[1]]);
});
it("Will find a number of closest nodes in a node list, excluding itself.", function () {
    ...
    var calculator = new DistanceCalculator_1.DistanceCalculator(nodes);
    expect(calculator.findXClosestNodesToNodeInList(new Node_1.Node(0, 0), nodes, 2)).toStrictEqual([nodes[0], nodes[1]]);
});
